...
C	 
C	PTENSOR 
      CALL STIFFNESS(AMUO,CELLV(IPHASEN),ANU,PC(IPHASEN,:,:,:,:))      
      CALL INVERSEFOURTH(PC(IPHASEN,:,:,:,:),TEMP41)
...
C TOTAL STRESS
          ...
          END IF
          CALL Aijkl_Bkl(HC,DTSTRAIN,TEMP25)
          CALL Aij_PLUS_Bij(TEMP25,TEMP21,TEMP26)
          CALL Aij_PLUS_Bij(TSTRESS,TEMP26,TSTRESS)
C D TENSOR
          AAP(:,:,:,:)=0.0D0
          DO I=1,IPHASEN
              CALL Aijmn_Bmnkl(PAO(I,:,:,:,:),PTENSOR,TEMP41)
          ...
          DO I=1,IPHASEN
              ...
              CALL Aijmn_Bmnkl(TEMP42,PTENSOR,TEMP43)
...
C
                      ...
                      CALL Aijmn_Bmnkl(PAO(I,:,:,:,:),PTENSOR,TEMP46)
     ...
     1 TEMP410)
                      ...
                      CALL Aijmn_Bmnkl(TEMP413,PTENSOR,TEMP414)
...
C PHASE STRESS AND STRAIN
     ...
     1 3,TEMP23)
          ...
          END DO 
          DO I=1,(IPHASEN-1)
              CALL CHEMOCELL(CELLV(I),EGSTRAIN(I,:,:),PSTRESS(I,:,:),
...
C          WRITE(6,*) TSTRESS(3,3) 
		  ...
		  WRITE(6,*) 'TIME=',TIME(2)		  
          WRITE(6,*) 'TVOID=',TVOID
		  WRITE(6,*) 'EGSTRAIN=',EGSTRAIN(1,1,1)
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
C                               OUTPUT
C-----------------------------------------------------------------------
...
C CONSTANTS
      DATA ZERO,HALF,ONE,TWO,FOUR,PI,DSO,VCTEST
